...
function buildApp(name, buildCmd, distDir, outputDir) {
    ...
    execSync(`cd ../apps/${name} && ${buildCmd}`, { stdio: 'inherit', cwd: __dirname });
    copyDir(
        path.join(__dirname, `../apps/${name}/${distDir}`),
        path.join(docsDir, outputDir)
    );
}



console.log('Preparing index.js...');
// Copy index.css to / public
const indexJsSrc = path.join(__dirname, '..', 'packages', 'ui', 'src', 'index.js');
const indexJsDest = path.join(__dirname, '..', 'public', 'index.js');
if (fs.existsSync(indexJsSrc)) {
    fs.copyFileSync(indexJsSrc, indexJsDest);
    console.log('index.js copied to /public');
} else {
